...
import PlayerSearchCard from '../native/components/PlayerSearchCard';
class SearchContainer extends Component {
  ...
  render = () => {
    ...
    const playerCards = allPlayers.map((playerStats, key) => {
      return <PlayerSearchCard player={playerStats} key={key} />
    })

    const searchCards = searchResults.map((playerStats, key) => {
      return <PlayerSearchCard player={playerStats} key={key} />
    })
